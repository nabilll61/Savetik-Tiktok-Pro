...
export default function FeedbackSection({ onBack, showToast }: FeedbackSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!content.trim()) {
      showToast('Ceritain detailnya terlebih dahulu!', 'warning');
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        showToast(data.message || 'Feedback berhasil dikirim!', 'success');
        setContent('');
        setContact('');
      } else {
        showToast(data.message || 'Gagal mengirim feedback.', 'error');
      }
    } catch (err) {
  ...
  return (
    <section className="py-6 px-4 md:px-8 max-w-2xl mx-auto space-y-6 animate-in fade-in duration-200">
