...
import { Heart, ArrowLeft, MessageSquare, ExternalLink, ShoppingBag, Download } from 'lucide-react';
...
export default function DonationSection({ onBack, showToast }: DonationSectionProps) {
  ...
  const triggerDownload = async () => {
    ...
    try {
      ...
      if (showToast) {
        showToast('Gagal mengunduh otomatis, silakan tekan lama gambar lalu simpan.', 'warning');
      }
      // Fallback to direct anchor link download approach
      const link = document.createElement('a');
    ...
    <div className="max-w-3xl mx-auto px-4 py-8" id="donation-section">
      ...
      <section className="mb-12">
        ...
        <div className="mb-8 max-w-[280px] sm:max-w-[320px] mx-auto flex flex-col gap-3">
          <div 
            className="relative border-[3px] border-neo-border rounded-[15px] overflow-hidden shadow-[5px_5px_0px_0px_#E11D48] bg-neo-card cursor-pointer group select-none active:scale-[0.98] transition-transform"
            onMouseDown={handlePressStart}
            onMouseUp={handlePressEnd}
            onMouseLeave={handlePressLeave}
            onTouchStart={handlePressStart}
            onTouchEnd={handlePressEnd}
            title="Tekan lama untuk mengunduh"
          >
            <img 
              src={donationBanner} 
              alt="Donation Banner" 
            ...
            <div className="absolute inset-x-0 bottom-0 bg-black/75 py-1.5 text-center text-[10px] text-white font-mono font-bold opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            ...
            className="w-full py-3 bg-[#10B981] hover:bg-[#059669] text-white font-heading font-black text-xs md:text-sm uppercase tracking-wider border-[3px] border-neo-border shadow-[4px_4px_0px_0px_var(--neo-border)] active:translate-y-0.5 active:shadow-[1px_1px_0px_0px_var(--neo-border)] transition-all flex items-center justify-center gap-2 cursor-pointer rounded-[8px]"
          >
