...
app.get('/robots.txt', (req, res) => {
  ...
  if (fs.existsSync(robotsPath)) {
    ...
    res.setHeader('Content-Type', 'text/plain');
    res.send("User-agent: *\nAllow: /\nSitemap: https://nabil-official.web.id/sitemap.xml");
  }
});
...
app.get('/sitemap.xml', (req, res) => {
  ...
  if (fs.existsSync(sitemapPath)) {
    ...
    res.setHeader('Content-Type', 'application/xml');
    res.send('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://nabil-official.web.id/</loc></url></urlset>');
  }
});
